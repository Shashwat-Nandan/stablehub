...
import { subscriberDb, newsletterDb } from '../database.js';
...
function createTransporter() {
  ...
  // Support both SMTP_ and EMAIL_ prefixed env vars
...
router.post('/send', authenticateToken, async (req, res) => {
  try {
    ...
    const emailPromises = recipients.map(subscriber => {
      const mailOptions = {
        from: process.env.SMTP_FROM || process.env.EMAIL_USER || 'newsletter@stablehub.com',
        to: subscriber.email,
        subject: subject,
        html: `
          <!DOCTYPE html>
          ...
          <body>
            <div class="container">
              ...
              <div class="footer">
                ...
                <p><a href="#">Unsubscribe</a> | <a href="${process.env.APP_URL || 'http://localhost:3001'}">Visit Website</a></p>
              </div>
            </div>
          </body>
          </html>
